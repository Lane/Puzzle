function point_tests() {
	...
	test('_calculateRadius(originOffset)', function() {
		var pt = new Point();
		equal(pt._calculateRadius({x: 100, y: 100}), 141, "Radius correctly calculated");
	});
	...
	test('_calculateAngle(originOffset)', function() {
		...
		equal(pt._calculateAngle({x: -100, y: -150}),124,"Correctly calculated angle: 124 degrees");
	...
	test('setOrigin()', function() {
		...
		pt.setOrigin({x: -100, y: -300});
		equal(pt.getRadius(), 283,"Radius correct after setting origin point to [-100, -300]");
		...
		equal(pt.getRadius(), 762,"Radius correct after setting origin point to [-600, 200]");
		equal(pt.getAngle(), 23,"Angle correct after setting origin point to [-600, 200]");
	});
